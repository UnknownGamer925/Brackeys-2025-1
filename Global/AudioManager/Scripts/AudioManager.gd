...
func create_2d_audio_at_location(location: Vector2, type: SoundEffect.SOUND_EFFECT_TYPE) -> void:
	if sound_effect_dict.has(type):
		...
		if sound_effect.has_open_limit():
			...
			new_2D_audio.pitch_scale += MainManager.rng.randf_range(-sound_effect.pitch_randomness, sound_effect.pitch_randomness )
...
func create_audio(type: SoundEffect.SOUND_EFFECT_TYPE) -> void:
	if sound_effect_dict.has(type):
		...
		if sound_effect.has_open_limit():
			...
			new_audio.pitch_scale += MainManager.rng.randf_range(-sound_effect.pitch_randomness, sound_effect.pitch_randomness )
